...
function episodeKitHtml(g, { item, reach = {}, clips = [], insights = [] } = {}) {
  ...
  const guestLinksBlock = guestLinks.length
    ? `<div class="ep-kit-guest-links ep-kit-guest-links--top fade-up">
        <span class="ep-kit-links-label">More from ${esc(g.name)}</span>
${guestLinks.map((link) => `        <a href="${esc(link.href)}" target="_blank" rel="noopener noreferrer" class="share-btn share ep-kit-guest-link">${esc(link.label)}</a>`).join("\n")}
      </div>`
    : "";
